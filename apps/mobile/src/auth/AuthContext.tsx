...
async function acceptSession(tokens: AuthTokens) {
  ...
  useEffect(() => {
    void (async () => {
      try {
        ...
        if (!stored) return;
        const { data } = await api.POST('/auth/refresh', { body: { refreshToken: stored } });
        if (data) {
        ...
        // offline / api down — stay signed out silently
      } finally {
        setRestoring(false);
      }
    })();
